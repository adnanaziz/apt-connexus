...
public class Upload extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			// APT: the BlobKey is not a url, so we use ImagesServiceFactory to
			// get
			// a URL to serve the image stored in the blob from
			ImagesService imagesService = ImagesServiceFactory
					.getImagesService();
			String bkUrl = imagesService.getServingUrl(blobKey);
			log.warning("bkUrl = " + bkUrl);
			// String bkUrl =
			// "http://users.ece.utexas.edu/~adnan/adnan-new.JPG";
			...
			log.warning("entering new ConnexusImage");
			ConnexusImage s = new ConnexusImage(streamId, streamName, content, bkUrl);
			log.warning("s = " + s.toString());
			ofy().save().entity(s).now();
			log.warning("added image to datastore, about to redirect");
			// after the creation of the image is complete we want to return to
			// the
			// stream that it was added to
			// we use the streamId as an argument to the ShowStream jsp
			// NOTE: in the "/ShowStream..." below if we left off the leading /
			// we
			// woud
